...
#ifdef DEBUG_NET_IP6
# define IP6DEBUG(a...)  printf("ip6: " a)
#else
# define IP6DEBUG(a...)
#endif
...
uip_neighbor_out(void)
{
  ...
  u16_t linklocal = HTONS(0xfe80); /* FIXME this should be FE80::/10 */

  /* Check if the destination address is on the local network.
   * FIXME, for the moment we assume a 64-bit "netmask" */
  if (memcmp(IPBUF->destipaddr, &linklocal, 2) == 0) {
    /* Remote address is link local address, send directly. */
    uip_ipaddr_copy(ipaddr, IPBUF->destipaddr);
  }
  else if(!uip_ipv6addr_netwk_cmp((uip_ipaddr_t*)IPBUF->destipaddr, &uip_hostaddr, uip_prefix_len)) {
