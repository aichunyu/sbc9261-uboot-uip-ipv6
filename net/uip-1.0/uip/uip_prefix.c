...
int
ip6_masklen (uip_ipaddr_t netmask)
{
  int len = 0;
  unsigned char val;
  unsigned char *pnt;
...
void
masklen2ip6 (int masklen, uip_ipaddr_t *netmask)
{
  unsigned char *pnt;
  int bit;
  int offset;

  memset (netmask, 0, sizeof (uip_ipaddr_t));
  pnt = (unsigned char *) netmask;
...
void
apply_mask_ipv6 (int prefixlen, uip_ipaddr_t *v6addr)
{
  unsigned char *pnt;
  int index;
  int offset;
