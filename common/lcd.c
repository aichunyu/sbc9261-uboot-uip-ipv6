...
#elif defined(CONFIG_MACH_PM9261)
...
# endif /* CONFIG_LCD_INFO */
	...
	debug ("info: %s\n", info);
	lcd_drawchars (LCD_INFO_X, LCD_INFO_Y, (u_char*)info, strlen(info));
	debug ("lcd_drawchars done\n");

	sprintf (info, "(C) 2006 Ronetix");
	debug ("info: %s\n", info);
	lcd_drawchars (LCD_INFO_X, LCD_INFO_Y + VIDEO_FONT_HEIGHT,
		       (u_char*)info, strlen(info));
	debug ("lcd_drawchars done\n");

	sprintf (info, "support@ronextix.at");
	debug ("info: %s\n", info);
	lcd_drawchars (LCD_INFO_X, LCD_INFO_Y + VIDEO_FONT_HEIGHT * 2,
		       (u_char*)info, strlen(info));
	debug ("lcd_drawchars done\n");
	...
	sprintf (info, "AT91SAM9261 CPU at %s MHz", "240");
	lcd_drawchars (LCD_INFO_X, LCD_INFO_Y + VIDEO_FONT_HEIGHT * 3,
		       (u_char*)info, strlen(info));
