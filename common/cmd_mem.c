...
#ifdef CONFIG_HAS_DATAFLASH
	/* Check if we are writting to DataFlash */
	if (addr_dataflash (addr)){
		puts ("Unsupported destination.\n\r");
		return 1;
	}
#endif
	...
	*/
	...
	if ( (addr2info(dest) != NULL)
...
#endif
		{
			...
			printf ("Copy %i bytes to Flash... ", count*size);
...
#ifdef CONFIG_HAS_DATAFLASH
	/* Check if we are copying from Flash to DataFlash */
	if (addr2info (addr) && addr_dataflash(dest)){
		puts ("Unsupported combination of source/destination.\n\r");
		return 1;
	}

	/* Check if we are copying from RAM to DataFlash */
	if (addr_dataflash(dest) && !addr_dataflash(addr)){
		int rc;

		printf ("Copy %i bytes to DataFlash... ", count*size);
...
#endif
		int rc;
		rc = read_dataflash(addr, count * size, (uchar *) dest);
		if (rc != 1) {
			dataflash_perror (rc);
			return (1);
		}
