...
unsigned long flash_init (void)
{
	...
	AT91C_BASE_SYS->SMC_SETUP0  = SMC_SETUP0_VAL;
   	AT91C_BASE_SYS->SMC_PULSE0	= SMC_PULSE0_VAL;
	AT91C_BASE_SYS->SMC_CYCLE0	= SMC_CYCLE0_VAL;
	AT91C_BASE_SYS->SMC_CTRL0	= SMC_CTRL0_VAL;
...
ulong flash_get_size (vu_short *addr, flash_info_t *info)
{
	...
	default:
		debug ("Unknown Manufacturer ID: %04X\n", value);
	...
	case (AMD_ID_MIRROR & 0xFFFF):
		...
		case (AMD_ID_GL064N_2 & 0xFFFF):
			if (addr[15] == (AMD_ID_GL064N_3 & 0xffff)) {
				printf ("Chip: S29GL064N-R2\n");
				info->flash_id += FLASH_S29GL064M;
				info->sector_count = 128;
				info->size = 0x00800000;
				for (i = 0; i < info->sector_count; i++) {
			...
			} else if (addr[15] == (AMD_ID_GL032N_3 & 0xffff)) {
				printf ("Chip: S29GL032N-R2\n");
				info->flash_id += FLASH_S29GL064M;
